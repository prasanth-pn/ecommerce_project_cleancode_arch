...
func (c *userDatabase) ListProductsByCategories(category_id int, pagenation utils.Filter) ([]domain.ProductResponse, utils.Metadata, error) {
	var products []domain.ProductResponse
	query := `SELECT COUNT(*) OVER(), P.product_id,P.product_name,P.description,P.image_path,P.price,C.category_name FROM products AS P INNER JOIN categories AS C ON P.category_id=C.category_id WHERE P.category_id=$1 LIMIT $2 OFFSET $3;`
	...
	for rows.Next() {
		var product domain.ProductResponse
		err = rows.Scan(&totalrecords, &product.Product_Id, &product.Product_Name, &product.Description, &product.Image, &product.Price, &product.Category_Name)
		if err != nil {
			return nil, utils.Metadata{}, err
		}
		products = append(products, product)
...
func (c *userDatabase) UpdateCart(totalprice float32, quantity, product_id, user_id uint) error {
	...
	err := c.DB.QueryRow(query, quantity, totalprice, time, product_id, user_id)
	fmt.Println(err, "inthe reposidtory")
	return err.Err()
...
func (c *userDatabase) ViewCart(user_id uint) ([]domain.CartListResponse, error) {
	...
	query := `SELECT C.quantity,C.total_price,P.description,P.image_path,P.product_name,C.product_id
	FROM products AS P
...
func (c *userDatabase) Insert_To_My_Order(carts domain.CartListResponse, order_id string) error {
	query := `INSERT INTO orderd_items(product_id,order_id,product_name,image_path,price,quantity)
	 values($1,$2,$3,$4,$5,$6);`
	err := c.DB.QueryRow(query, carts.Product_id, order_id, carts.Product_Name, carts.Image_Path, carts.Total_Price, carts.Quantity)
	return err.Err()
